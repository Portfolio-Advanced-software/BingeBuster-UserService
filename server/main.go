...
import (
	...
	mongodb "github.com/Portfolio-Advanced-software/BingeBuster-UserService/mongo"
...
func (s *UserServiceServer) CreateUser(ctx context.Context, req *userpb.CreateUserReq) (*userpb.CreateUserRes, error) {
	...
	user.Id = oid.Hex()
	// return the blog in a CreateUserRes type
...
func (s *UserServiceServer) ListUsers(req *userpb.ListUserReq, stream userpb.UserService_ListUsersServer) error {
	// Initiate a User type to write decoded data to
	data := &models.User{}
...
func (s *UserServiceServer) UpdateUser(ctx context.Context, req *userpb.UpdateUserReq) (*userpb.UpdateUserRes, error) {
	...
	return &userpb.UpdateUserRes{
		User: &userpb.User{
			Id:            decoded.ID.Hex(),
			Watchedmovies: decoded.WatchedMovies,
		},
	}, nil
}
...
func main() {
	...
	userdb = db.Database("MovieHistoryService").Collection("Watched")
